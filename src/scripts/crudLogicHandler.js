...
class ProjectManager {
  ...
  static deleteList(pIndex, lIndex) {
    ...
    if (project) project.deleteList(lIndex);
  }
  ...
  static deleteTask(projectIndex, listIndex, taskIndex) {
    ...
    if (project) project.deleteTaskFromList(listIndex, taskIndex);
  }
}
...
function addNewList(newListName) {
  ...
  localStorage.setItem(
    `project_${active_project}`,
    projectList()[active_project].toJSON()
  );
}
...
function deleteList(projectIndex, listIndex) {
  ...
  localStorage.setItem(
    `project_${index}`,
    projectList()[projectIndex].toJSON()
  );
}
